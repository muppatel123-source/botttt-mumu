    ...
    async slashExecute(interaction) {
        try {
            ...
            return interaction.reply({
                content: '❌ Failed to list free agents.',
                ephemeral: true
            });
        }
    }
...
async function runFreeAgents({
    ...
    const lines = players.map((player, index) => {
        const mention = player.discordID ? `<@${player.discordID}>` : '`No Discord Linked`';
    ...
    const embed = new EmbedBuilder()
        ...
        .setFooter({
            text: `Total free agents: ${players.length}`
        })
        .setTimestamp();

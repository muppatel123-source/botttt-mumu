...
async function runChangeTeamName({
    guild,
    actorId,
    oldName,
    newName,
    ...
    if (!cleanNewName || cleanNewName.length < 2 || cleanNewName.length > 50) {
        return reply({
            content: '❌ New team name must be between 2 and 50 characters.'
        });
    }
    ...
    const duplicate = await Team.findOne({
        guildId: guild.id,
        name: {
            $regex: new RegExp(`^${escapeRegex(cleanNewName)}$`, 'i')
        }
    });

    if (duplicate) {
        return reply({
            content: `❌ A team named **${cleanNewName}** already exists.`
        });
    }

    let team = null;
    const organizer = await isOrganizer(guild, actorId);

    if (oldName) {
        if (!organizer) {
            return reply({
                content: '❌ Only organizers can rename another team by old name.'
            });
        }
        ...
        team = await Team.findOne({
            guildId: guild.id,
            name: {
                $regex: new RegExp(`^${escapeRegex(normalizeName(oldName))}$`, 'i')
            }
        });
    } else {

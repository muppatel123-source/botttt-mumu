...
async function runRelease({ guild, actorId, targetUser, reply }) {
    ...
    /* ── Remove captain/player Discord roles ── */
    const releasedMember = await guild.members.fetch(targetUser.id).catch(() => null);
    if (releasedMember) {
        ...
        for (const rid of captainRoleIds) {
            if (releasedMember.roles.cache.has(rid)) {
                await releasedMember.roles.remove(rid).catch(() => null);
            }
        }
    }
